...
// Angka Ascending (Harga)
void sortHargaAsc(CuciMotor *pelanggan, int jumlah){
    for(int i=0;i<jumlah-1;i++){
        for(int j=0;j<jumlah-i-1;j++){
            if(pelanggan[j].harga > pelanggan[j+1].harga){
                swap(pelanggan[j], pelanggan[j+1]);
            }
        }
    }
...
// Metode Bebas (Selection Sort - Plat)
void sortPlatAsc(CuciMotor *pelanggan, int jumlah){
    for(int i=0;i<jumlah-1;i++){
        int minIndex = i;
        for(int j=i+1;j<jumlah;j++){
            if(pelanggan[j].plat < pelanggan[minIndex].plat){
                minIndex = j;
            }
        }
...
int main(){
    ...
    do{
        ...
        if(menu == 1){
            ...
            sortNamaDesc(pelanggan, jumlah);
            cout<<"Berhasil sorting nama (Descending)\n";
        }
        else if(menu == 7){
            sortHargaAsc(pelanggan, jumlah);
            cout<<"Berhasil sorting harga (Ascending)\n";
        }
        else if(menu == 8){
            sortPlatAsc(pelanggan, jumlah);
            cout<<"Berhasil sorting plat (Selection Sort)\n";
        }
